...
| `gamesetup.html` | **The "Table" screen.** Two states: (A) config — name, player count, per-seat AI/Human + difficulty, modes; (B) inline **host waiting room** (code, share, live slot list, Cancel). The game cannot start until every Human seat has joined (no "fill with AI" shortcut). Solo/all-AI games go straight to playgame; any Human seat creates the game in-place (via `src/host.js`) and shows state B. The former separate `creategame.html` page was merged in here (June 2026) — no more redundant second name prompt. **The 5-8 count buttons live behind an opt-in disclosure** (`#exp-toggle` → `#exp-panel`, Aug 2026) that carries the experimental caveat; 2/3/4 are the always-visible default. Collapsing the panel while a 5-8 count is selected **falls back to 4** on purpose — otherwise the game starts at a count whose only warning is off-screen. The buttons keep a dashed border once selected for the same reason. |
...
| `rules.html` | Standalone rules page. **Documents 2-4 players only** (Aug 2026) — every 5-8P mention was removed (meta description, the Objective's player-count line, the "3 rows" build-step branch, the four 5-8 setup-table rows, and the two-copies note). 5-8P is experimental and reachable only through `gamesetup.html`'s disclosure, so the rules must not advertise it. The `.rules-footer` carries three buttons — Cards List, **Print Proof** (→ `print-proof.html`, `target="_blank" rel="noopener"` since Sep 2026: it is a side trip off the rules, not the next thing to read, so a reader mid-rules shouldn't lose their scroll to it; the other two ARE onward destinations and stay same-tab), Play Digital Version; at 375px they wrap 2+1 with the primary CTA on its own row. `cardslist.html` and `print-proof.html` still mention 5-8 players — deliberately unresolved, see the note under the 5-8 Player Support section. Audited July 2026 against `docs/RULEBOOK_WRITING_STANDARDS.md` — scorecard + remaining to-do list in `docs/RULES_PAGE_AUDIT.md`. Section `id`s + the `.rules-toc` contents panel are load-bearing: the panel is a `<nav>`, so it must keep its explicit `display`/`gap` resets or `css/style.css`'s bare `nav`/`nav ul` header rules turn it into a flex row. Store act bands use **three distinct hues — dark green / purple / tan** (Aug 2026, PB: "simple and distinct foremost"), replacing the three-brown lightness ramp, which was correct for ordinal data but forced the reader to decode which brown was which against the key. **Blue, yellow and red stay permanently off-limits** — they are the suit colours and these cells are card BACKS, which are printed in exactly those colours, so a blue band reads as "this band is River". Act 3/Act 2 are only ~9 L\* apart, so the **45°/-45°/horizontal hatch is load-bearing, not decorative** — it is the only thing separating them in greyscale. Each band is also labelled in place (`.band-tag`), and `.pyramid-grid`'s `padding-right` must stay ≥ the longest rendered tag or it overflows at 375px (the tags are absolutely positioned, so the grid otherwise centres as if they weren't there). The key carries **four** entries: the three bands plus **face-up** — it previously read "Act 1 (front, face-up)", conflating the 2-row Act 1 band with the single face-up front row. See the comment block in `css/rules-page.css`, and `mockups/act-band-colours.html` for the rendered proof incl. the greyscale check. **`.band-tag` must clear the brick stagger, not just the row it hangs off** (Sep 2026): the tag sits on a band's FIRST row, which is never the offset one, so a bare `left: 100%` landed half a card short of the staggered row below — and that row is transformed (a stacking context), so it painted over the tag and ate its first letter ("ACT 3 — BACK" read "CT 3 — BACK"). Fixed by `left: calc(100% + var(--brick-shift))`, with `--brick-shift: 12.5px` on `.pyramid-grid` now feeding BOTH the tag offset and `.brick-offset`'s `translateX` so they cannot drift. `padding-right` went 106px → **119px**, and the longest tag clears it by only **1.3px at 375px** — re-measure if any tag's wording gets longer. **Sequence-of-Play headings carry an `h3 .phase-mode` caption** naming the mode ("1. Draw — everyone at the same time" / "2. Buy — one player at a time, in turn order"); it uses Georgia because rules.html loads **only** Playfair Display, so any other family silently falls back anyway, and it deliberately has no `white-space: nowrap` (the Buy caption is wider than a 375px column and must wrap, not overflow). **Scoring the drawn Cows lives under Draw, not Buy** — it is the outcome of the draw and happens whether or not you buy. **The uncovering demo is THREE frames, one card leaving per frame** (Sep 2026) — two frames read as though two cards could leave the Store at once, and you take exactly one per turn; the middle frame (one gone, still covered, nothing flips) is the whole point, so don't collapse it back to save width. **All three frames must sit on ONE row at desktop width** — a three-step sequence wrapped 2+1 stops reading as a sequence. The column is a fixed 712px (`.rules-page` is capped at 760px), and the demo needed 749px, so the spacing was tightened: column gap `1.25rem → 0.7rem`, `.reveal-arrow` `1.4rem → 1.15rem`, caption cap `13rem → 11.5rem`, plus shorter captions. Now 634px used, ~85px of slack, and the block is 110px tall instead of 231px. `flex-wrap` stays so it still stacks on a phone. **The outline marks the SUBJECT, and the subject never changes** — it sits on back 2 in all three frames, so the captions' "it" points at something visible and the covered → face-up change reads on the card being tracked. Outlining the two *coverers* instead (the earlier version) left "it" referring to an unmarked card and made the highlight mean something different in each frame; don't move it back to the front row. ⚠️ **Which cards are marked is GEOMETRY, and the old version was wrong.** With the front row staggered half a card right, front card `j` overlaps back cards `j` and `j+1`, so in a 3+3 excerpt: back 0 ← front 0 **only** (the overhang end card), back 1 ← front 0+1, back 2 ← front 1+2. Only **back 2** can have both its coverers removed without incidentally uncovering something else — the demo takes front 1 then front 2 and flips back 2. The previous version marked front 0+1 and revealed back 1, which should also have flipped back 0. Re-derive that table before moving any mark; `.reveal-demo`'s frames are verified by replicating `isCardCovered`'s overlap test against each frame's rendered rects. **`.reveal-demo .pyramid-grid` zeroes `padding-right`** — the demo reuses `.pyramid-grid` for geometry but has no `.band-tag`, so the 119px label reservation was padding each 72px frame out to 208px. **Version line convention:** `v3.N` tracks the RULES (bump only when a player must DO something differently — it is what someone checks against a printed insert), while "last updated" tracks the DOCUMENT and bumps for any noticeable change. Sep 9 2026 was clarifications only, so v3.1 stood and only the date moved. The Objective lede was also rewritten: "lays out their whole deck … and counts their Cows" read as though the deck count WAS the score, when the Herd is banked round by round and the showdown adds to it — say "bank" and "on top of" explicitly, and keep that in step with `#showdown` and its Jo example. |
...
| `print-proof.html` | **Print layout proof for the physical rules insert.** `noindex`'d working file, **linked from `rules.html`'s footer as "Print Proof"** since Aug 2026 (it was unlinked before that). Consequence: it is now a player-visible page while still carrying dev controls (panel size / body type / overflow verdict) and printer's imposition labels. **5-8 player content was stripped Aug 2026** to match `rules.html` (panel 1 cover meta → "2-4 players", the two-copies clause and the "or 3 rows (5-8)" branch on panel 4, and the four 5-8 rows off panel 5's table). **FORMAT COMMITTED + PANELS RE-CUT (Sep 2026, PB).** Two decisions, both closed:

1. **The 8-panel poker accordion is final.** The format switcher (50mm cross fold / 100mm reference) is deleted and `--panel-w/h/margin/cols` in `:root` are fixed values, not defaults. Only `--body-pt` is still UI-driven, and only because type size is still being validated: **6pt and 7pt hold all 8 panels; 8pt overflows 4 and 9pt overflows 6** — 7pt is the ceiling, not a taste call.
2. **The Store-size table moved onto the Setup panel**, which is what filled the near-empty panel it used to occupy (previously flagged here as "leave it" — superseded). The table dropped its **Rows** column (6 at every count, now stated once in build step 3) and its **Total** column (just Per Act × 3), leaving Players / Per row / Per Act. Panels renumbered: 5 Draw+Buy, 6 Uncovering, 7 Final Scoring, **8 is now a back cover** — an accordion is 4 panels × 2 sides and cannot be 7, so the merge had to release a whole panel.

Making it fit took 8.7mm off panel 4, and where it came from is deliberate: the redundant "4 players — 9 per row" caption (4.2mm, the table says it), and the Store diagram shrunk to `--pc-h: 4.2mm` (**diagram height = pc-h × 4.3** — six rows overlapping 34% — so it is the cheapest millimetre on the panel; the CSS comment carries the full 5.0/5.7/4.2 ladder). A new `.panel > :last-child { margin-bottom: 0 }` was needed because a trailing margin still counts toward `scrollHeight`: the panel measured 0.7mm clear on content yet the checker read 1mm over.

Two measurement traps: `report()` now counts panels from the DOM rather than a hardcoded 8 (it lied the moment panels moved), and `scrollHeight` reports only *overflow* on these fixed-height `overflow:hidden` panels — it reads 0mm free on a panel that is 84% empty, so sum the children's bottoms against the padding-box top instead.

**Store diagram brought in line with `rules.html` (Sep 2026, PB).** The two surfaces now
render the same picture, on both axes that were out of step:

- **Colours** — the retired three-brown ramp (`#241610`/`#7d5636`/`#c19464`) is replaced by the
  web rules' `#1f3d2b` / `#57406e` / `#c19464` (dark green / purple / tan). Every constraint in
  `css/rules-page.css`'s comment block now applies here too — **blue/yellow/red stay off-limits**
  (they are the suit colours and these cells are card BACKS), and the 45°/-45°/horizontal hatch is
  **load-bearing, not decorative**: Act 3 and Act 2 are only ~9 L\* apart, so the hatch is the only
  separation in greyscale. ⚠️ This does commit the printed insert to **green and purple ink** —
  that press decision is still PB's, and if it comes back to a limited palette both files change
  together.
- **Overlap** — `.prow + .prow` is `-0.4375 × pc-h`, copied from rules.html's `-14px` on its 32px
  card (was `-0.34`). Six rows are therefore `pc-h × 3.8125`, **not × 4.3** — the old multiplier is
  quoted all over the panel-4 comment and every measurement in that ladder predates this change.
  The tighter stagger shrank the 4.2mm diagram 18.1mm → 16.0mm, so **panel 4 now has ~2.8mm of
  slack, not ~1mm.** Left unspent on purpose: it is the fit margin for a body-type change.
- **Paint order** — `.prow` now carries `position: relative`, which is why rules.html's
  `.pyramid-row` has it too. It is not layout: `.prow.off` sets a `transform`, a transform creates
  a **stacking context**, and a transformed element paints with the positioned descendants while a
  plain non-positioned row paints its background much earlier. So every offset row floated above
  every non-offset row — rows 1→2 looked fine, then row 3 (purple) hid *behind* row 2 (green) and
  row 5 (tan) behind row 4, which is backwards for a brick stack whose front row must cover the
  back. Making all six rows positioned puts them in one paint layer where tree order decides.
  Verified by pixel scan: the lower row went from 69px visible to 342px on the row-2/3 boundary.
- The `.bandkey` swatches moved from inline `style="background:…"` to `.sw.b3/.b2/.b1`, sharing the
  band rules, so a swatch **cannot** lose its hatch — the same trap `css/rules-page.css` warns
  about, where the `background` SHORTHAND silently resets `background-image`.

**Panel 5 phase headings (Sep 2026).** `1. Draw` / `2. Buy` now carry an `h3 .mode` caption naming
the mode — *everyone at once* vs *one at a time, in turn order* — and the "add the Cows you drew to
your Herd" line **moved up under Draw**, where it belongs: it is the result of the draw, not
something the buy phase awards. Paying for it needed the redundant "Busted players sit this phase
out" bullet dropped (the Bust callout already says "no buy"). Panel 5 slack: **5.1mm**.

**`rules.html` and `print-proof.html` are maintained SEPARATELY (PB, Sep 2026).** They cover the
same rules, but do NOT chase one to match the other's wording, and do not add a parity rule between
them — the same fix is simply made twice, in each file's own voice and space budget. A parity
...
No `rev`/`Revision` label sits in front of the date on either surface (PB) — an ISO date already
reads as a date and the abbreviation just looked odd. Panel 8 reads "Preliminary draft · 2026-09-09";
the banner reads "Last updated 2026-09-09".

**Panel 6's uncovering demo is THREE frames too**, with the same geometry rule as `rules.html`
(front `j` covers back `j` and `j+1`; take front 1 then front 2, flip back 2 — see that file's
entry for the full table and why marking front 0+1 is wrong). Three frames did not fit at the old
5mm card, so `.reveal .pyr` gets its own `--pc-w: 3.4mm / --pc-h: 4.85mm` and the captions drop
from a 32mm cap to 15mm — at three across, a caption only gets a third of the panel.
...
> **⚠️ DEMOTED TO EXPERIMENTAL, Aug 2026.** The engine is unchanged and every mechanic below still
> works — what changed is how it is *presented*. 5-8P has had no live-MP QA pass at the table, its
> AI tiers were only first measured in July 2026, and the 9-row Store is cramped on a phone. So:
> `rules.html` no longer documents it at all (it is a 2-4 player rulebook now), and
> `gamesetup.html` hides the 5-8 count buttons behind an opt-in "Show 5-8 player games ·
> Experimental" disclosure carrying the caveat.
>
> **Two mentions were deliberately left alone** — decide before "cleaning" them up:
> `cardslist.html`'s closing note ("At 5-8 players you shuffle two copies of each Act deck") and
> `print-proof.html`, the layout proof for the **physical** rules insert, whose panels still carry
> the 5-8 rows and the two-copies line. print-proof is a print-format decision, not a web copy
> one; changing it changes what gets printed.
