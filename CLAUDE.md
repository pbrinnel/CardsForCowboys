...
| `sim/AI_SEARCH_RESULTS.md` | **The verdict (B5).** Flat-MC buy-phase search beats the pros under a FAIR (human-equivalent) info model — AI may not see hidden deck order; rollouts determinize each deck. Clears the +5pp bar at **N=256** (Δ2P +7.9 / Δ4P +11.6 vs best pro); N=64 only clears 4P. Fairness costs ~3pp + ~4× rollouts. MP-deterministic by construction. Shipping = port resumable engine into play.js + live timing gate (separate phase). Margin real but moderate vs port cost — a product call. |
